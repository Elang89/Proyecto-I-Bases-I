...
CREATE OR REPLACE PACKAGE pet_search_package AS
    ...
    FUNCTION edit_myPets(user_id person.person_id%type)
       RETURN SYS_REFCURSOR; 
    FUNCTION all_pets
       RETURN SYS_REFCURSOR;
    ...
    FUNCTION find_myAdoptedPets(user_id person.person_id%type)
         RETURN SYS_REFCURSOR; 
    FUNCTION all_returned_pets
       RETURN SYS_REFCURSOR;   
    FUNCTION month_search(month NUMBER)
       RETURN SYS_REFCURSOR;
...
CREATE OR REPLACE PACKAGE BODY pet_search_package AS
    ...
    ORDER BY pet.pet_date DESC;
      ...
      END;   
       ...
       BEGIN
        OPEN my_pets FOR select pet.pet_code, pet_name, pet_type_name, pet_race_name, pet_cond_name, pet_energy_level, pet_learn_skill, vet_name, person_name, petlocation, petnotes, petabandondescription, pet_space, pet_treatment, pet_color, pet_sickness_name, pet_med_name, image
    from pet, pettype, petrace, petCondition, petSize,petEnergy, petlearningskill, veterinary, person, petSpace, pettreatments, petcolor, petsickness, petmedicine, pet_photo_storage
    where (owner_id = user_id OR adoptant_id = user_id)
    AND pettype.pet_type_code = pet.pet_type_code
    AND petrace.pet_race_code = pet.pet_race_code
    AND pet.pet_cond_code = petcondition.pet_cond_code
    AND pet.pet_size_code = petsize.pet_size_code
    ...
    OPEN pet_search_result FOR select pet.pet_code,pet_name, pet_type_name, pet_race_name, pet_cond_name, pet_energy_level, pet_learn_skill, vet_name, person_name, petlocation, petnotes, petabandondescription, pet_space, pet_treatment, pet_color, pet_sickness_name, pet_med_name, image
    from pet, pettype, petrace, petCondition, petSize,petEnergy, petlearningskill, veterinary, person, petSpace, pettreatments, petcolor, petsickness, petmedicine, pet_photo_storage
    ...
    FUNCTION find_myAdoptedPets(user_id person.person_id%type)
     ...
     EXCEPTION 
      ...
      RETURN NULL;
       ...
       RETURN SYS_REFCURSOR
       IS
          allReturned SYS_REFCURSOR;
       BEGIN
        OPEN allReturned FOR select pet.pet_code,pet_name, pet_type_name, pet_race_name, pet_cond_name, pet_energy_level, pet_learn_skill, vet_name, person_name, petlocation, petnotes, petabandondescription, pet_space, pet_treatment, pet_color, pet_sickness_name, pet_med_name, image, return_count
    from pet, pettype, petrace, petCondition, petSize,petEnergy, petlearningskill, veterinary, person, petSpace, pettreatments, petcolor, petsickness, petmedicine, pet_photo_storage
    where pettype.pet_type_code = pet.pet_type_code 
    AND pet.return_count > 0
    AND petrace.pet_race_code = pet.pet_race_code
    AND pet.pet_cond_code = petcondition.pet_cond_code
    AND pet.pet_size_code = petsize.pet_size_code
    AND pet.pet_energy_code = petEnergy.Pet_Energy_Code
    ...
    ORDER BY pet.pet_date DESC;
      ...
      FUNCTION month_search(month NUMBER)
       RETURN SYS_REFCURSOR
       IS
          month_pets SYS_REFCURSOR;
       BEGIN
        OPEN month_pets FOR select pet.pet_code, pet_name, pet_type_name, pet_race_name, pet_cond_name, pet_energy_level, pet_learn_skill, vet_name, person_name, petlocation, petnotes, petabandondescription, pet_space, pet_treatment, pet_color, pet_sickness_name, pet_med_name, image
    from pet, pettype, petrace, petCondition, petSize,petEnergy, petlearningskill, veterinary, person, petSpace, pettreatments, petcolor, petsickness, petmedicine, pet_photo_storage
    where pet_date <= add_months(TRUNC(SYSDATE) + 1, -month)  
    AND pet.adoptant_id IS NULL
    AND pettype.pet_type_code = pet.pet_type_code
    AND petrace.pet_race_code = pet.pet_race_code
    AND pet.pet_cond_code = petcondition.pet_cond_code
